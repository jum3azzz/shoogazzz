...
export async function findGameByCode(gameCode) {
  /*
   * The host's real document ID is generated by
   * generateGameId() and has no relation to the
   * six-character code shown on screen.
   *
   * A guest only has the code, so we query the
   * "games" collection for the document whose
   * gameCode field matches, and read the real
   * document ID off the result (snapshot.id).
   */
  ...
  if (snapshot.empty) {
    throw new Error("No waiting game found with that code.");
  }
